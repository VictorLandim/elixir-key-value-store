defmodule KVServer.Command do
  @doc """
  Runs the given command on a given pid.
  """
  def run(command, pid)

  def run({:create, bucket}, server) do
    case KV.Router.route(bucket, server, :create, [server, bucket]) do
  ...
  defp lookup(bucket, server, callback) do
    case KV.Router.route(bucket, server, :lookup, [server, bucket]) do
      {:ok, pid} -> callback.(pid)
